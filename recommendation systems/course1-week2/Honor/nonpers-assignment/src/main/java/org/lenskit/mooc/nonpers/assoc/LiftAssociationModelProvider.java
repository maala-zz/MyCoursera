    ...
    public AssociationModel get() {
        ...
        for (Long2ObjectMap.Entry<LongSortedSet> xEntry: itemUsers.long2ObjectEntrySet()) {
            ...
            for (Long2ObjectMap.Entry<LongSortedSet> yEntry: itemUsers.long2ObjectEntrySet()) {
                ...
                while( it.hasNext() ){
                    if( xUsers.contains(it.next()) )Join++ ;
                }
                double enm = new Double(Join)*new Double(allUsers.size()) ;
                double den = new Double(xUsers.size())*new Double(yUsers.size()) ;
                itemScores.put(yId,enm/den) ;
            }
